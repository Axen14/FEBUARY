...
const LoanManager = () => {
    ...
    //     // Ensure membersName is not undefined before searching
    //     if (!membersName || membersName.length === 0) {
    //         console.warn("Warning: membersName is empty or undefined!");
    //         return;
    //     }
    
    //     console.log("Members Data:", membersName);
    
    //     const matchedMember = membersName.find((member) => {
    //         if (member.first_name && member.middle_name && member.last_name) {
    //             const fullName = `${member.first_name} ${member.middle_name} ${member.last_name}`.trim().toLowerCase();
    //             console.log("Checking:", fullName, "vs", selectedName.toLowerCase());
    //             return fullName === selectedName.toLowerCase();
    //         } 
    //         if (member.name) {
    //             console.log("Checking:", member.name.trim().toLowerCase(), "vs", selectedName.toLowerCase());
    //             return member.name.trim().toLowerCase() === selectedName.toLowerCase();
    //         }
    //         return false;
    //     });
    
    //     console.log("Full Member Data:", matchedMember);
    
    //     if (matchedMember && matchedMember.accountN) {
    //         console.log("Matched Account Number:", matchedMember.accountN);
    //         setLoanData((prev) => ({
    //             ...prev,
    //             account: matchedMember.accountN, 
    //         }));
    //     } else {
    //         console.log("No matching account found or accountN is undefined!");
    //         setLoanData((prev) => ({ ...prev, account: "" }));
    //     }
    // };
    useEffect(() => {
    ...
    const handleHolderChange = (e) => {
            ...
            console.log("🔍 Account Holder Selected:", searchValue);
        
            // Find the matching member based on the full `name`
            ...
            console.log("🛠️ Available Members:", membersName); // ✅ Debug all members
        
            if (selectedMember) {
                console.log("✅ Matched Account Holder:", selectedMember);
                console.log("📌 Account Number Found:", selectedMember.accountN || "❌ Not Found");
        
                setLoanData(prev => ({
                    ...prev,
                    account_holder: searchValue,
                    account: selectedMember.accountN || "", // ✅ Ensure it doesn’t break if undefined
                    share_capital: selectedMember.share_capital || ""
                }));
            } else {
                console.warn("⚠️ No matching account holder found.");
                setLoanData(prev => ({
                    ...prev,
                    account_holder: searchValue,
                    account: "",
    ...
    useEffect(() => {
        const fetchMembers = async () => {
            try {
                const response = await axios.get('http://localhost:8000/api/members/get-members/');
                                ...
                                <div className="form-group d-flex justify-content-between">
                                    ...
                                    {makerTwoSearch.length > 0 && (
                                            <div className='maker-name-list'>
                                                {makerTwoSearch.map((maker) => (
                                                    <button onClick={() => {
                                                        setLoanData({ ...loanData, co_make_2: maker.name })
                                                        setMakerOneSearch([])
                                                    }}
                                                        key={maker.name}>{maker.name}</button>
                                                ))}
                                ...
                                <div className="form-group d-flex justify-content-between">
                                    ...
                                    {makerThreeSearch.length > 0 && (
                                            <div className='maker-name-list'>
                                                {makerThreeSearch.map((maker) => (
                                                    <button onClick={() => {
                                                        setLoanData({ ...loanData, co_maker_3r: maker.name })
                                                        setMakerOneSearch([])
                                                    }}
                                                        key={maker.name}>{maker.name}</button>
                                                ))}
