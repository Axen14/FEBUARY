...
def member_login(request):
    ...
    member = User.objects.get(username=account_number)
    if member is not None:
        from rest_framework_simplejwt.tokens import RefreshToken


        member_profile = Member.objects.get(email = member.email)
        account = Account.objects.get(account_holder = member_profile.memId)
        account_number = account.account_number
        tokens = RefreshToken.for_user(member)
        print(f"DEBUG: Retrieved account_number={member}")
...
class MemberProfileView(APIView):
    ...
    def get(self, request):
        try:
            ...
            account = Account.objects.get(account_number=account_number)
            print("account holder::", account.account_holder_id)
            member = Member.objects.get(memId=account.account_holder_id)
...
def get_members(request):
    ...
    data = [{'name': f"{user.first_name} {user.middle_name} {user.last_name}"} for user in users]
...
class PaymentScheduleViewSet(viewsets.ModelViewSet):
    ...
    def payment_schedule_summaries(self, request):
        ...
        summaries = summaries.annotate(
            ...
            total_balance=F('loan__outstanding_balance'),
    ...
    def mark_as_paid(self, request, pk=None):
        schedule = self.get_object()
        received_amount = request.data.get('received_amount', schedule.balance)
        # schedule.process_payment(received_amount)
        return Response({'status': f'Payment processed and marked as paid. {received_amount}'}, status=status.HTTP_200_OK)
...
#         # return JsonResponse({'status': f'Payment processed and marked as paid.'}, status=status.HTTP_200_OK)
#         return JsonResponse({'status': f"{request}"}, status=status.HTTP_200_OK)
...
def mark_as_paid(request, id):
    ...
    try:
        ...
        schedule.process_payment(received_amount,account_number)

        return JsonResponse({'status': 'Payment processed and marked as paid.'}, status=200)
...
def update_breakdown(request):
    ...
    for schedule in payment_schedules:
        #allen
        print(f"Before Update - Schedule {schedule.id}: Principal={schedule.principal_amount}, Payment={schedule.payment_amount}")
        # Save the original principal only on the first update
        if schedule.original_principal is None:
            schedule.original_principal = schedule.principal_amount #allen
        
        schedule.payment_amount = new_amount
        schedule.principal_amount = new_amount
        schedule.save()
    #allen   
        print(f"After Update - Schedule {schedule.id}: Principal={schedule.principal_amount}, Payment={schedule.payment_amount}")
     
    # Update loan term in Loan model
    loan = payment_schedules.first().loan
    total_loan_amount = loan.loan_amount
    new_installment = new_amount * 2  # Bi-monthly payments
    new_term = math.ceil(total_loan_amount / new_installment)  # Calculate new term
    
    if loan:
        print(f"Before Update - Loan {loan.control_number}: Principal={loan.loan_amount}, Period={loan.loan_period}")
    else:
        print("Error: Loan object not found")
    
        # Validate new loan term before saving
    if loan.loan_period_unit == 'months' and new_term > 48:
        return Response({"error": "Loan period cannot exceed 48 months for regular loans."}, status=400)
    if loan.loan_period_unit == 'years' and new_term > 4:
        return Response({"error": "Loan period cannot exceed 4 years for regular loans."}, status=400)

    loan.loan_period = new_term
    loan.loan_amount = new_amount
    loan.save()   #/allen

    print(f"After Update - Loan {loan.control_number}: Principal={loan.loan_amount}, Period={loan.loan_period}")

    return Response({"message": "Payment schedules updated successfully"}, status=200)
