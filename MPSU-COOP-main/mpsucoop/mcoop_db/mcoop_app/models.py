...
class Account(models.Model):
    ...
    def dec_balance(self, bal):
        self.shareCapital = self.shareCapital - bal
        self.save()
...
class Loan(models.Model):
    ...
    control_number = models.CharField(
        primary_key=True, max_length=5, unique=True, editable=False
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if 'outstanding_balance' not in kwargs.get('update_fields', []):
            ...
            self.takehomePay = abs( self.loan_amount - (
        ...
        if 'outstanding_balance' in kwargs.get('update_fields', []):
            super().save(*args, **kwargs)
            return
    ...
    #
    #     if 'outstanding_balance' in kwargs.get('update_fields', []):
    #         # Skip calculations that overwrite the outstanding_balance
    #         pass
    #     else:
    #         # Your other existing logic here
    #         if not self.system_settings:
    #             self.system_settings = SystemSettings.get_settings()
    #             print(f"Loaded system settings: {self.system_settings}")
    #
    ...
    def calculate_initial_values(self):
        ...
        self.outstanding_balance = abs(self.takehomePay)
    ...
    def yearly_recalculation(self):
        ...
        if years_elapsed >= 1:
            ...
            self.outstanding_balance = abs(self.loan_amount - (
...
class  PaymentSchedule(models.Model):
    ...
    #     received_amnt = Decimal(received_amount)
    #     if received_amnt <= 0:
    ...
    #         # print(f"paymount payment:: {self.payment_amount}")
    #         overpayment = received_amnt - self.payment_amount
    #         self.advance_pay += overpayment
    #         # print("advance payment: ", self.advance_pay)
    ...
    #         self.under_pay = Decimal('0.00')
    #
    #     self.balance -= received_amnt
    #     self.received_amnt = received_amnt
    #     if self.balance <= Decimal('0.00'):
    #         self.is_paid = True
    #
    #     self.save()
    def process_payment(self, received_amount,account_number):
        loans = PaymentSchedule.objects.filter(loan=self.loan, is_paid=False).order_by('due_date')

        # print(f"acoount number:: {account_number}")
        if self.balance <= Decimal("0.00"):
            return
        received_amnt = Decimal(received_amount)
        if received_amnt <= 0:
        ...
        self.balance -= received_amnt
        self.received_amnt = received_amnt
        if received_amnt >= self.principal_amount:
            self.is_paid = True
            self.balance  = self.balance - received_amnt
            # account = Member.objects.get(account_number=account_number)
            # Retrieve the Loan instance associated with the provided account number
            # account = Loan.objects.get(account__account_number=f"{account_number}")

            self.loan.update_outstanding(received_amnt)
            self.loan.account.save()

            print(f"OUTSTANDING BALANCE now: {self.loan.outstanding_balance}")
            self.save()
            for l in loans:
                if received_amnt <= 0:
                    break
                if not l.is_paid and self.id != l.id:
                    if received_amnt < l.principal_amount:
                        l.principal_amount -= received_amnt
                        received_amnt = Decimal('0.00')  # Fully used up
                        l.save()
                        break
                    else:
                        received_amnt -= l.principal_amount
                        l.principal_amount = Decimal('0.00')
                        l.is_paid = True
                        l.save()
    class Meta:
        ordering = ['due_date']
